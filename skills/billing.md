...
When the customer asks for a Mainland China tax invoice (fapiao), including a VAT special invoice, clearly include this policy in the customer draft, adapting it to the customer's language:

```text
Please note that the service provider for this transaction, LangGenius, is a non-PRC entity. Therefore, we're unable to issue a tax invoice under the PRC tax system, including a VAT special invoice. We can only provide the commercial Invoice generated by our billing system.
```

Do not suggest that downloading an Invoice/receipt or updating the Billing Portal can produce a PRC tax invoice. If the request also concerns an already-issued commercial Invoice, continue with the existing-invoice guidance below.
