...
- 跨会话上下文：每次收到外部客户邮件（包括已有会话回复和新 Front 会话），必须按同一标准化发件邮箱读取其他近期 Front 会话的已发送往来、状态和已有 Linear 链接；即使旧会话没有本地自动化状态也要读取。换主题或新 conversation ID 不视为全新案例，必须先理清多会话上下文，不得重复草稿、重复建单或作出矛盾回复。内部关联评论保持简短，仅写主记录/关联会话链接、已有 Linear 链接及未重复建草稿状态，不复制历史或当前邮件正文
